...
const NFTItem = ({
  ...
  React.useEffect(() => {
    setVote({
      ...voters.find((v) => v.publicKey === publicKey && v.mint === mint),
    });
  ...
  return (
    <Container pos="relative" style={{ position: "relative" }}>
      {imgExist && (
        <NFTImg
          src={metaData?.image}
          alt={`No Assets for ${metaData?.name}`}
      ...
      >
        <Button
          colorScheme="blue"
          variant="outline"
          isDisabled={vote && vote.vote === 1}
          onClick={() => onKeep()}
          isLoading={isKeeping}
          loadingText={`Keeping(${keeps})`}
        ...
        <Button
          colorScheme="blue"
          variant="outline"
          isDisabled={vote && vote.vote === 0}
          onClick={() => onBurn()}
          isLoading={isBurning}
          loadingText={`Burning(${burns})`}
          spinnerPlacement="end"
        >
          Burn({burns})
        </Button>
